...
class RelationExtractor:
    ...
    def create_training_data(self, texts: List[str], entities_list: List[List[Dict]], 
                           relations_list: List[List[Dict]]):
        """创建训练数据"""
        # 这里需要实现训练数据准备逻辑
        pass
...
if __name__ == "__main__":
    ...
    for rel in relations:
        print(f"{rel['subject']} --{rel['relation']}--> {rel['object']} (置信度: {rel['confidence']:.2f})")